...
const CODE_TASKS = new Set(['debug', 'refactor', 'generate', 'review', 'test', 'optimize', 'document']);
// ─── Hardware detection ───────────────────────────────────────────────────────
let _hwCache = null;
export function detectHardware() {
    ...
    const hasGpu = !!(process.env['CUDA_VISIBLE_DEVICES'] || process.env['ROCR_VISIBLE_DEVICES'] ||
        process.env['UTOE_GPU'] || process.env['METAL_DEVICE_WRAPPER_TYPE']);
    let ollamaCapacity = 'none';
    if (hasGpu && totalMemGB >= 16)
        ollamaCapacity = 'large';
    else if (hasGpu && totalMemGB >= 8)
        ollamaCapacity = 'medium';
    else if (totalMemGB >= 16)
        ollamaCapacity = 'medium';
    else if (totalMemGB >= 8)
        ollamaCapacity = 'small';
    else
        ollamaCapacity = 'tiny';
    _hwCache = { totalMemGB, cpuCount, hasGpu, ollamaCapacity };
    return _hwCache;
}
function getOllamaModelForCapacity(cap) {
    switch (cap) {
...
// ─── Scoring ──────────────────────────────────────────────────────────────────
function scoreCandidate(provider, model, weights, adaptiveStats) {
    ...
    return ((weights['savings'] ?? 1) * savings -
...
export class ModelRouter {
    plan(task, inputTokens, config = {}, opts = {}) {
        ...
        const effortLevel = (opts['effort'] ?? config['effort'] ?? 'medium');
        const effortTier = EFFORT_TO_TIER[effortLevel];
        const baseTier = effortTier ?? (TASK_TIERS[task] ?? 'medium');
        const tier = effortLevel === 'high'
            ? 'large'
            : effortLevel === 'low'
                ? 'cheap'
                : (inputTokens > 10000 ? 'large' : inputTokens < 300 ? 'cheap' : baseTier);
        const isCode = CODE_TASKS.has(task);
        const candidates = [];
        ...
        for (const [provider, isAvail] of Object.entries(available)) {
            ...
            if (provider === 'ollama') {
                ...
                if (tier === 'large' && hw.ollamaCapacity === 'large')
            ...
            candidates.push({
                provider: provider, model, tier, score,
    ...
    select(task, inputTokens, config = {}) {
    ...
    getHardwareProfile() { return detectHardware(); }
    _buildFallbackChain(task, policy, available) {
        const chains = policy?.routing?.fallback_chains ?? {};
        const raw = chains[task] ?? chains['default'] ?? [
        ...
        return raw.filter((e) => available[e.provider]);
    }
    _buildForcedPlan(task, inputTokens, config, policy, opts) {
        const available = getAvailableProviders(config);
        const provider = (config.forceProvider ?? this._detectProvider(config));
    ...
    _compressionLevel(task, policy) {
        if (['debug', 'refactor', 'generate'].includes(task))
            return 'lossless';
