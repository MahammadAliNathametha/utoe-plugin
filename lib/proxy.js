...
// Compresses tool_result content blocks BEFORE forwarding to Anthropic.
// Addresses the real-world issue of 50k+ token tool-call payloads.
function compressToolResults(messages, compressFn) {
    return messages.map(msg => {
        ...
        const newContent = msg.content.map(block => {
            ...
            if (raw.length < 500) return block; // not worth compressing
            const result = compressFn(raw, { lossless: false, aggressiveCode: false });
...
// ─── Rate-limit failover ──────────────────────────────────────────────────────
// When Anthropic returns 529 (overloaded) or 429 (rate limited), auto-routes
// the request to a free alternative: Groq → Gemini → Ollama.
async function failoverRequest(body, config, compress) {
    const failoverChain = [
        config.GROQ_API_KEY && {
            name: 'groq',
        ...
        config.GEMINI_API_KEY && {
            name: 'gemini',
            url: `https://generativelanguage.googleapis.com/v1beta/openai/chat/completions?key=${config.GEMINI_API_KEY}`,
            auth: null,
    ...
    // Convert Anthropic messages format to OpenAI
    const msgs = [
    ...
    for (const provider of failoverChain) {
        try {
            ...
            const res = await fetch(provider.url, {
                method: 'POST',
                headers,
                body: JSON.stringify({ model: provider.model, messages: msgs, max_tokens: body.max_tokens ?? 2048 }),
            ...
            if (!text) continue;
            return { provider: provider.name, text, model: provider.model };
        } catch { continue; }
    }
    return null;
}
...
// ─── Peak-hour detection ──────────────────────────────────────────────────────
// Returns true during Anthropic peak hours (5am–11am PT = 13:00–19:00 UTC)
function isAnthropicPeakHour() {
...
export async function createHonoApp(config) {
    ...
    app.post('/v1/messages', async (c) => {
        ...
        // Compress tool_result blocks — fixes the 50k-token tool-call problem
        const compressedChatMessages = compressToolResults(chatMessages, compress);

        // Build forwarded request body
        const forwardBody = {
            ...body,
            messages: compressedChatMessages,
            ...(systemMsg !== undefined && { system: systemMsg }),
            // Apply tighter token limit during Anthropic peak hours
            ...(isAnthropicPeakHour() && !body.max_tokens && { max_tokens: 4096 }),
        };
        ...
        // Handle Anthropic overload (529) or rate-limit (429) — failover to free providers
        let failoverUsed = null;
        if (upstreamRes.status === 529 || upstreamRes.status === 429) {
            const fo = await failoverRequest(forwardBody, config, compress);
            if (fo) {
                failoverUsed = fo.provider;
                c.header('X-UTOE-Failover', fo.provider);
                c.header('X-UTOE-Failover-Model', fo.model);
                globalStats.totalRequests++;
                globalStats.byProvider[fo.provider] = (globalStats.byProvider[fo.provider] ?? 0) + 1;
                return c.json({
                    id: `msg_utoe_${Date.now()}`,
                    type: 'message',
                    role: 'assistant',
                    content: [{ type: 'text', text: fo.text }],
                    model: fo.model,
                    stop_reason: 'end_turn',
                    utoe: { failover: fo.provider, reason: `anthropic_${upstreamRes.status}` },
                }, 200);
            }
        }
    ...
    // ── Native: GET /stats ──────────────────────────────────────────────────────
    app.get('/stats', (c) => {
        const uptimeSec = (Date.now() - globalStats.startedAt) / 1000;
        const burnRate = uptimeSec > 0
            ? Math.round((globalStats.totalTokensIn / uptimeSec) * 60)
            : 0;
        return c.json({
            memory: memory.stats(),
            cache: semanticCache.stats(),
            pipeline: globalStats,
            burn_rate: { tokens_per_min: burnRate, peak_hour: isAnthropicPeakHour() },
        });
    });
    // ── Native: POST /forget ────────────────────────────────────────────────────
