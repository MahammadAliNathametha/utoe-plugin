...
// ─── Exports ──────────────────────────────────────────────────────────────────
export function suggestBetterPrompt(rawPrompt) {
...
export function scorePrompt(prompt) {
    ...
    if (prompt.trim().startsWith('{'))
        score += 20;
    if (/output|format|style/i.test(prompt))
        score += 10;
    if (/max_\w+|limit/i.test(prompt))
        score += 5;
    if (detectLanguage(prompt))
        score += 5;
    if (prompt.length > 50 && prompt.length < 500)
        score += 10;
    if (/^(hey|hi|hello)/i.test(prompt))
        score -= 10;
    if (/please|kindly|could you/i.test(lower))
        score -= 10;
    if (/\bi\s+need\s+you\b/i.test(lower))
        score -= 5;
...
//# sourceMappingURL=prompt-suggester.js.map