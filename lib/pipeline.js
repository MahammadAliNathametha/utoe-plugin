...
import { suggestBetterPrompt as suggestPrompt, detectIntent } from './prompt-suggester.js';
import { enforceTokenEnvelope } from './token-guard.js';
import { commandEngine, createDefaultSession } from './command-engine.js';
/**
 ...
 */
export async function optimizeMessages(input, config = {}, session = createDefaultSession()) {
    ...
    const enrichedInput = memoryFacts.length > 0
        ? `[Memory Context]\n${memoryFacts.join('\n')}\n\n[User Prompt]\n${cleanedInput}`
        : cleanedInput;
    const enrichedMessages = messages.map(m => m === lastUserMsg ? { ...m, content: enrichedInput } : m);
 ...
 */
export async function runPipeline(input, config = {}, session = createDefaultSession()) {
    ...
    commandEngine.onResponseReceived(rawInput, execution.text, inputTokens, outputTokens, compressionStats.savedTokens, session);
    return {
...
function detectTopic(text) {
    ...
    if (/\b(function|class|import|export|const|let|var|def|return|async|await|bug|error|refactor|test)\b/.test(t))
        return 'code';
    if (/\b(sql|query|database|table|schema|csv|json|pandas|dataframe|analytics)\b/.test(t))
        return 'data';
    if (/\b(docker|kubernetes|k8s|deploy|ci\/cd|pipeline|terraform|aws|gcp|azure|nginx|server)\b/.test(t))
        return 'infra';
    if (/\b(write|essay|article|blog|draft|email|report|summarize|translate)\b/.test(t))
        return 'writing';
    return 'general';
}
/**
 ...
 */
export function suggestBetterPrompt(prompt) {
