...
export function compressMessages(messages, opts = {}) {
    ...
    // 4. Jaccard near-duplicate history dropping
    // Measures token overlap between each user turn and all prior user turns.
    // If similarity > 0.50 (same question rephrased), the older turn is
    // replaced with a compact reference — preserving intent, dropping tokens.
    const jaccardTerms = (text) => {
        const STOP = new Set(['the','and','for','that','this','with','from','are','not','but',
            'you','all','can','has','its','was','will','been','have','they','what','when',
            'which','just','your','code','like','also','then','them','than','use','please']);
    ...
    messages = messages.map((msg, idx) => {
        if (msg.role !== 'user') { seenTermSets.push(null); return msg; }
        const content = typeof msg.content === 'string' ? msg.content : '';
        if (content.length < 40) { seenTermSets.push(null); return msg; }
        const terms = jaccardTerms(content);
        const dupAt = seenTermSets.findIndex(prev => prev !== null && jaccardSim(terms, prev) > 0.45);
        seenTermSets.push(terms);
        if (dupAt >= 0) {
            const approxSaved = Math.round(content.length / 4);
            totalSaved += approxSaved;
            return { ...msg, content: `[UTOE: near-duplicate of turn ${dupAt + 1} (${approxSaved} tokens saved)]` };
        }
        return msg;
    });

    // 5. Per-message compression
