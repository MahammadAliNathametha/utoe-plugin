 ...
 */
export function runPrePromptHook(input, effortLevel = 'medium', appendSystemPrompt) {
 ...
 */
export function runPostPromptHook(output, projectRoot = process.cwd()) {
...
export class TerminalHookManager {
     ...
     */
    async runPreHooks(input, session) {
        let current = input;
        for (const hook of this._preHooks) {
            try {
                current = await hook(current, session);
            }
            catch { /* ignore */ }
        }
        return current;
     ...
     */
    async runPostHooks(output, session) {
        let current = output;
...
function compressDockerPs(text) {
    const lines = text.split('\n').filter(Boolean);
    if (lines.length <= 5)
        return text;
...
function compressStackTrace(text) {
    const lines = text.split('\n');
    const errorLines = [];
    const frameLines = [];
    let inFrames = false;
...
function compressTestOutput(text) {
    ...
    if (lines.length <= 30)
        return text;
    const keep = lines.filter((l) => /(?:PASS|FAIL|ERROR|✓|✗|×|●)/i.test(l) ||
...
//# sourceMappingURL=terminal-hook.js.map