 ...
 */
export function detectAITool(projectRoot = process.cwd()) {
    ...
    if (envTool === 'aider')
        return { tool: 'aider', contextFile: 'CONVENTIONS.md', label: 'Aider' };
    if (envTool === 'continue')
    ...
    if (/cursor/i.test(proc))
        return { tool: 'cursor', contextFile: '.cursorrules', label: 'Cursor' };
    if (/claude/i.test(proc))
        return { tool: 'claude', contextFile: 'claude.md', label: 'Claude Code' };
    if (/windsurf/i.test(proc))
        return { tool: 'windsurf', contextFile: '.windsurfrules', label: 'Windsurf' };
 ...
 */
export function bootstrapProjectFiles(projectRoot = process.cwd()) {
    const created = [];
    const skipped = [];
    ...
    if (!fs.existsSync(contextFilePath)) {
        try {
            if (!fs.existsSync(contextFileDir))
 ...
 */
export function enforceClaudeMdLimit(projectRoot = process.cwd()) {
 ...
 */
export function loadRelevantSupportFiles(query, projectRoot = process.cwd(), topK = 3) {
    ...
    for (const fname of fs.readdirSync(logsDir)) {
        ...
        try {
            ...
            if (score > 0.05)
                results.push({ file: fname, content: content.slice(0, 800), score });
        }
        catch { /* ignore */ }
    }
...
export function patchShellProfile(port = 8787) {
    ...
    if (/bash/.test(shell))
        candidates.push(path.join(home, '.bashrc'), path.join(home, '.bash_profile'));
    candidates.push(path.join(home, '.zshrc'), path.join(home, '.bashrc'), path.join(home, '.bash_profile'), path.join(home, '.profile'));
    // Deduplicate
    const seen = new Set();
    const profiles = candidates.filter(p => { if (seen.has(p))
        return false; seen.add(p); return true; });
    ...
    for (const profile of profiles) {
        ...
        try {
            ...
            if (content.includes(marker))
                return { file: profile, added: false };
