...
export function trackRequest(stats, result) {
    stats.totalRequests++;
...
export function buildDashboardHTML(config, stats) {
...
<body>
  ...
  <div class="meta">
    <span>Universal Token Optimization Engine v1.2.0</span>
  ...
  <div class="grid">
    ...
    <div class="card" id="card-burnrate">
      <div class="label">Burn Rate</div>
      <div class="value" id="val-burnrate">—</div>
      <div class="sub" id="sub-burnrate">tokens / min</div>
    </div>
    <div class="card" id="card-failover" style="display:none">
      <div class="label">Rate Limit Guard</div>
      <div class="value yellow" id="val-failover">ON</div>
      <div class="sub" id="sub-failover">Peak hour detected</div>
    </div>
  </div>
  <div id="peak-warning" style="display:none;background:#422006;border:1px solid #fbbf24;border-radius:8px;padding:.75rem 1rem;margin-bottom:1rem;font-size:.8rem;color:#fbbf24">
    ⚠ <strong>Anthropic peak hours (1 pm–7 pm UTC)</strong> — UTOE rate-limit protection is active.
    If Claude returns 529 errors, UTOE auto-routes to Groq / Gemini / Ollama instead.
  </div>
...
<script>
  ...
  function applyStats(s, data) {
    data = data || { pipeline: s };
    ...
    // Burn rate + peak-hour guard
    const burnData = data && data.burn_rate ? data.burn_rate : null;
    const burnEl = document.getElementById('val-burnrate');
    const burnSub = document.getElementById('sub-burnrate');
    if (burnEl && burnData) {
      const bv = burnData.tokens_per_min;
      burnEl.textContent = fmt(bv);
      burnEl.style.color = bv > 5000 ? 'var(--yellow)' : bv > 2000 ? 'var(--accent)' : 'var(--green)';
      if (burnSub) burnSub.textContent = 'tokens / min' + (bv > 5000 ? ' — HIGH' : '');
    }
    const peakWarn = document.getElementById('peak-warning');
    const failoverCard = document.getElementById('card-failover');
    const isPeak = burnData?.peak_hour ?? false;
    if (peakWarn) peakWarn.style.display = isPeak ? 'block' : 'none';
    if (failoverCard) failoverCard.style.display = isPeak ? 'block' : 'none';
  }

  // Apply seed immediately
  applyStats(prev, { pipeline: prev });
