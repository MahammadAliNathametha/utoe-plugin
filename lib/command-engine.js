 ...
 */
export function buildFlagInjection(flags) {
    const parts = [];
 ...
 */
export function injectFlagsIntoMessages(messages, session, appendSystemPrompt) {
...
function handleCompact(session) {
...
function handleBtw(args, session) {
    if (!args) {
...
function handlePlanning(session) {
...
function handleRewind(args, session) {
...
function handleEffort(args, session) {
    const effortMap = {
        l: 'low', low: 'low',
...
function handleAppendSystem(args, session) {
    if (!args) {
        session.flags.append_system_prompt = null;
...
function handleForget(args, session, memoryFn) {
...
function handleSkills(projectRoot) {
...
export function handleEscPress(session) {
    const now = Date.now();
...
export class CommandEngine {
     ...
     */
    prepareMessages(messages, session, appendSystem) {
     ...
     */
    onResponseReceived(userInput, assistantResponse, inputTokens, outputTokens, savedTokens, session) {
    ...
    runDiagnostics() {
        ...
        // 2. Router Check
        const plan = router.plan('general', 100, {}, { effort: 'medium' });
