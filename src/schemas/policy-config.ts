...
export function assertPolicyConfig(policy: unknown): void {
  if (!isObject(policy)) throw new Error('Invalid policy: expected object');

  assertString(policy['policy_version'], 'policy.policy_version');

  if (!isObject(policy['hard_limits'])) throw new Error('Invalid policy.hard_limits');
  const hl = policy['hard_limits'] as Record<string, unknown>;
  ...
  if (!isObject(policy['quality'])) throw new Error('Invalid policy.quality');
  const q = policy['quality'] as Record<string, unknown>;
  ...
  if (!isObject(policy['task_profiles'])) throw new Error('Invalid policy.task_profiles');
  const tp = policy['task_profiles'] as Record<string, unknown>;
  for (const key of ['code', 'debug', 'summary', 'analysis']) {
    if (!isObject(tp[key])) throw new Error(`Invalid policy.task_profiles.${key}`);
    const profile = tp[key] as Record<string, unknown>;
  ...
  if (!isObject(policy['routing'])) throw new Error('Invalid policy.routing');
  const r = policy['routing'] as Record<string, unknown>;
  if (!isObject(r['objective_weights'])) throw new Error('Invalid policy.routing.objective_weights');
  const w = r['objective_weights'] as Record<string, unknown>;
  ...
  if (!isObject(r['fallback_chains'])) throw new Error('Invalid policy.routing.fallback_chains');

  if (!isObject(policy['memory'])) throw new Error('Invalid policy.memory');
  const m = policy['memory'] as Record<string, unknown>;
...
export function loadDefaultPolicy(policyPath = DEFAULT_POLICY_PATH): unknown {
