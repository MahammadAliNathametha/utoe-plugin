...
export type ProviderName =
  | 'ollama' | 'groq' | 'openai' | 'anthropic' | 'gemini'
  | 'mistral' | 'cohere' | 'together' | 'perplexity' | 'deepseek'
  | 'fireworks' | 'replicate' | 'openrouter' | 'anyscale' | 'lepton'
  | 'octoai' | 'cloudflare' | 'azure' | 'bedrock' | 'vertex'
  | 'moonshot' | 'qwen' | 'yi' | string;
...
export type TaskType =
  | 'summarize' | 'translate' | 'classify' | 'clean' | 'explain'
  | 'refactor' | 'review' | 'document' | 'debug' | 'generate'
  | 'analyze' | 'test' | 'optimize' | 'architect' | 'reason';
...
export interface CompressionStats {
  ...
  savedTokens: number;
  savedPct: number;
  layers: Array<{ name: string; saved: number }>;
  /**
...
export interface PipelineResult {
  ...
  telemetry: {
    ...
    attempts: Array<{ provider: ProviderName; model: string }>;
    estimatedCostUsd: number;
  };
}

// SessionState is defined in command-engine.ts (authoritative, includes flags + effort)
...
export interface UTOEConfig {
  ...
  // Execution (use mode field — UTOE_MODE is the env-var name, not a config key)
