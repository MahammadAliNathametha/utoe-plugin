...
const ENV_KEY_MAP: Partial<Record<string, string>> = {
...
async function callProvider(
  ...
  if (provider === 'gemini') return callGemini(messages, model);

  const ollamaBase = process.env['OLLAMA_URL'] ?? 'http://localhost:11434';
  if (provider === 'ollama') return callOpenAICompatible(ollamaBase, '', messages, model);
...
class LLMExecutor {
  async execute(
    messages: ChatMessage[],
    opts: { provider: ProviderName; model: string; fallbackChain: FallbackEntry[] }
    ...
    for (const entry of chain) {
      attempts.push({ provider: entry.provider as ProviderName, model: entry.model });
      try {
        const text = await callProvider(entry.provider as ProviderName, entry.model, messages);
        return {
          text,
