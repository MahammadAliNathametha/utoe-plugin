...
export class TelemetryStore {
  ...
  readRecent(limit = 100): TelemetryEvent[] {
    ...
    try {
      ...
      return lines
        ...
        .filter((e): e is TelemetryEvent => !!e);
    } catch {
      return [];
    }
  }
   ...
   */
  getRollups(): Record<string, { successRate: number; avgLatency: number; count: number }> {
    const events = this.readRecent(200);
    const stats: Record<string, { successes: number; total: number; latency: number }> = {};

    for (const e of events) {
      const key = `${e.provider}:${e.model}`;
      if (!stats[key]) stats[key] = { successes: 0, total: 0, latency: 0 };
      stats[key]!.total++;
      if (e.success) stats[key]!.successes++;
      stats[key]!.latency += e.latencyMs;
    }

    const result: Record<string, { successRate: number; avgLatency: number; count: number }> = {};
